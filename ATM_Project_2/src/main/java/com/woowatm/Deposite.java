...
public class Deposite extends JFrame {
	...
	public Deposite(String str, String pin, String accNo) {
...
//		accountHolder user = cs.getDetails(pin);
		...
		JLabel lblNewLabel_1 = new JLabel("Enter The Deposit Amount ");
		lblNewLabel_1.setForeground(new Color(0, 0, 0));
		lblNewLabel_1.setBackground(new Color(240, 240, 240));
		lblNewLabel_1.setFont(new Font("Times New Roman", Font.BOLD, 50));
		lblNewLabel_1.setBounds(271, 50, 598, 81);
		...
		JButton btnBanking_1 = new JButton("Deposit");
		btnBanking_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int amount = Integer.parseInt(textField.getText());
				CheckService cs = new CheckService();
				if (cs.depositMoney(accNo, amount))
					JOptionPane.showConfirmDialog(null, "Deposit!" + amount);
				else
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Deposite frame = new Deposite("ATM project", args[0], args[1]);

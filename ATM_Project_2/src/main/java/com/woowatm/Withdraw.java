...
public class Withdraw extends JFrame {
	...
	public Withdraw(String str, String pin, String accNo) {
...
//		accountHolder user = cs.getDetails(pin);
		...
		JLabel lblNewLabel_1 = new JLabel("Enter The Withdrawal Amount ");
		...
		JButton btnBanking_1 = new JButton("Withdraw");
		btnBanking_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int amount = Integer.parseInt(textField.getText());
				if (cs.withdrawMoney(accNo, amount))
					JOptionPane.showConfirmDialog(null, "Withdraw!" + amount);
				else
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Withdraw frame = new Withdraw("ATM project", args[0], args[1]);
